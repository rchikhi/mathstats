...
def generate(n, mu, sigma, gap, distribution="lognormal"):
	r=100 #readlength
...
def plot_estimate_uncertainty(n, mu, sigma, distribution="lognormal"):
	if distribution == 'lognormal':
		gaps = []
		estimates_log = []
		estimates_normal = []
		for gap in range(-2000,6001, 1000):
			for i in range(50):
				sample = generate(n, mu, sigma, gap)
				gap_est = lnpe.GapEstimator(mu, sigma, 100, sample, 1000000, c2_len=1000000)
				...
				gap_est_normal = pe.GapEstimator(mu_norm, sigma_norm, 100, mean_sample, 1000000, c2_len=1000000)
		...
		return None
	generate(n, mu, sigma, gap, distribution="lognormal")
